<?php
/**
 * Blogberg Widget Base Widget Class
 * @link http://codex.wordpress.org/Widgets_API#Developing_Widgets
 * @since Blogberg 1.0.0
 */

if( ! class_exists( 'Blogberg_Base_Widget' ) ):

class Blogberg_Base_Widget extends WP_Widget {
   ...
   public function get_html_dropdown_categories( $field ) {
       $dropdown = wp_dropdown_categories(
           array(
               ...
               'echo'              => 0,
               'show_option_none'  => esc_html__( '&mdash; Select &mdash;', 'blogberg' ),
               'option_none_value' => '0',
   ...
   public function get_html_dropdown_menus( $field ){
        ...
        ?>
            <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( $field[ 'id' ] ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( $field[ 'id' ] ) ); ?>">
                <option value="0"><?php echo esc_html__( '&mdash; Select &mdash;', 'blogberg' ); ?></option>
   ...
   public function get_menu( $id ){
        ...
        return wp_nav_menu( apply_filters( 'blogberg_widget_nav_menu_args', $nav_menu_args, $nav_menu, $id ) );
   }
