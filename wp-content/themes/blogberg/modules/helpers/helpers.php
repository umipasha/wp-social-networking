...
if( !function_exists( 'blogberg_enqueue' ) ):
/**
...
* @return void
*/
function blogberg_enqueue( $scripts ){

    # Do not enqueue anything if no array is supplied.
    if( ! is_array( $scripts ) ) return;

    $scripts = apply_filters( 'blogberg_scripts' , $scripts );
...
if( ! function_exists( 'blogberg_get_related_posts' ) ):
/**
* Get related posts
*
* @since Blogberg 1.0.0
...
* 
*/
function blogberg_get_related_posts( $taxonomy = array(), $per_page = 3, $get_params = false ){
...
* @return string	
*/	
function blogberg_modify_post_navigation_markup( $template, $class ){
...
add_filter( 'navigation_markup_template', 'blogberg_modify_post_navigation_markup', 10, 2 );

if( ! function_exists( 'blogberg_primary_menu_fb' ) ) :
/**
 ...
 * @return void
 */
function blogberg_primary_menu_fb( $arg ) {
    if( !$arg[ 'echo' ] ){
        ob_start();
    }
    ?>
    <ul>
        <li>
            <a href="<?php echo esc_url( home_url( '/' ) ) ?>"><?php esc_html_e( 'Home', 'blogberg' ) ?></a>
        </li>
    </ul>
    <?php
    if( !$arg[ 'echo' ] ){
...
if( !function_exists( 'blogberg_get_youtube_id' ) ):
/**
 ...
 * @return string | bool
 */
function blogberg_get_youtube_id( $url ) {
...
if( ! function_exists( 'blogberg_get_the_category' ) ):
/**
* Returns categories after sorting by term id descending
* 
* @since Blogberg 1.0.0
* @uses blogberg_sort_category()
* @return array
*/
function blogberg_get_the_category( $id = false ){
    $failed = true;
    ...
    if ( class_exists( 'WPSEO_Primary_Term' ) ){
        ...
        $blogberg_cat[0] = get_term( $wpseo_primary_term );

        if ( !is_wp_error( $blogberg_cat[0] ) ) { 
           $failed = false;
        }
    }
    ...
    if( $failed ){
      ...
      $blogberg_cat = get_the_category( $id );
      usort( $blogberg_cat, 'blogberg_sort_category' );  
    }
...
if( ! function_exists( 'blogberg_sort_category' ) ):
/**
* Helper function for blogberg_get_the_category()
*
* @since Blogberg 1.0.0
*/
function blogberg_sort_category( $a, $b ){
    return $a->term_id < $b->term_id;
}
endif;

if( ! function_exists( 'blogberg_explode_string_to_int' ) ):
/**
...
*/
function blogberg_explode_string_to_int( $str ){
    if( empty( $str ) )
        return false;
...
if( ! function_exists( 'blogberg_get_menu' ) ):
/**
* Returns wp nav
*
* @since Blogberg 1.0.0
...
* @return String
*/
function blogberg_get_menu( $location = 'primary', $echo = true ){
    ...
    switch( $location ){
        ...
        case 'primary':
            $args[ 'fallback_cb' ] = 'blogberg_primary_menu_fb';
            $args[ 'menu_id' ] = 'primary-menu';
            $args[ 'menu_class' ] = 'primary-menu';
            $menu = wp_nav_menu( apply_filters( 'blogberg_'.$location.'_menu', $args ) );
        break;

        case 'secondary':
            $args[ 'fallback_cb' ] = 'blogberg_secondary_menu_fb';
        ...
        case 'footer':
            $args[ 'fallback_cb' ] = 'blogberg_footer_menu_fb';
...
if( !function_exists( 'blogberg_hex2rgba' ) ):
/**
* Convert hexdec color string to rgb(a) string
* @since Blogberg 1.0.0
*/
function blogberg_hex2rgba($color, $opacity = false) {
